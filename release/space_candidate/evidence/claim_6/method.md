...
The implementation follows the released `mira` default `norm=False`: 100
centers are sampled uniformly in `[0,1]^12288`, one of 64 posterior draws
defines each radius, strict interior counts exclude the reference through the
strict comparison, and the released finite-sample probability is divided by
its maximum `64/65`. Squared Euclidean distances preserve the same comparisons.
